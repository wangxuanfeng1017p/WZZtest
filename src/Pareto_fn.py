...
def pareto_step(w, c, G):
    ...
    e = np.asmatrix(np.ones(np.shape(w)))  # (K, 1)
    m_up = np.hstack((GGT, e))  # (K, K+1)
    m_down = np.hstack((np.transpose(e), np.asmatrix(np.zeros((1, 1)))))  # (1, K+1)
...
def pareto_fn(w_list, c_list, model, num_tasks, loss_list):
    ...
    for i,loss in enumerate(loss_list):
        for p in model.parameters():
            if p.grad is not None:
                ...
                # Create a zero grad tensor on the same device as the parameter.
                grads[i].append(torch.zeros_like(p, device=p.device).view(-1))
    ...
    # NumPy 2.0 removed np.mat; use asmatrix/ndarray instead.
    weights = np.asmatrix(np.asarray(w_list, dtype=float)).reshape(-1, 1)
    c_mat = np.asmatrix(np.asarray(c_list, dtype=float)).reshape(-1, 1)
